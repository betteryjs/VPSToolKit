...
#	Description: Snell v5 管理脚本
#	功能：安装/卸载/更新/管理 Snell Server v5
#=================================================
...
# Snell v5 相关路径
SNELL_DIR="/etc/snell"
SNELL_FILE="/usr/bin/snell5-server"
SNELL_CONF="${SNELL_DIR}/snell5-server.conf"
SERVICE_FILE="/etc/systemd/system/snell5.service"
...
check_installed_status(){
	[[ ! -e ${SNELL_FILE} ]] && echo -e "${Error} Snell v5 没有安装，请检查！" && exit 1
}

check_status(){
	status=$(systemctl status snell5 2>/dev/null | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
	[[ -z "${status}" ]] && status="未运行"
}
...
check_new_ver(){
	...
	echo -e "${Info} 使用 Snell v5 版本 [ ${new_ver} ]"
}
...
set_port(){
	echo -e "请输入 Snell v5 监听端口 [1-65535]"
...
read_config(){
	[[ ! -e ${SNELL_CONF} ]] && echo -e "${Error} Snell v5 配置文件不存在！" && exit 1
...
SyslogIdentifier=snell5-server
Restart=on-failure
RestartSec=5
...
EOF
	...
	systemctl enable snell5.service
	echo -e "${Info} Snell v5 服务配置完成！"
}

install(){
	[[ -e ${SNELL_FILE} ]] && echo -e "${Error} 检测到 Snell v5 已安装！" && exit 1
...
view(){
	...
	if [[ "${ipv4}" != "IPv4_Error" ]]; then
		...
		echo -e "${Green_font_prefix}Snell = snell, ${ipv4}, ${port}, psk=${password}, version=5${Font_color_suffix}"
	fi
...
start(){
	...
	if [[ "$status" == "running" ]]; then
		echo -e "${Info} Snell v5 已在运行！"
	else
		systemctl start snell5
		sleep 2s
		check_status
		if [[ "$status" == "running" ]]; then
			echo -e "${Info} Snell v5 启动成功！"
		else
			echo -e "${Error} Snell v5 启动失败！"
		fi
	fi
	sleep 2s
}
...
stop(){
	check_installed_status
	check_status
	[[ "$status" != "running" ]] && echo -e "${Error} Snell v5 没有运行，请检查！" && exit 1
	systemctl stop snell5
	sleep 2s
	check_status
	if [[ "$status" == "running" ]]; then
		echo -e "${Error} Snell v5 停止失败！"
	else
		echo -e "${Info} Snell v5 停止成功！"
	fi
}

restart(){
	check_installed_status
	systemctl restart snell5
	sleep 2s
	check_status
	if [[ "$status" == "running" ]]; then
		echo -e "${Info} Snell v5 重启成功！"
	else
		echo -e "${Error} Snell v5 重启失败！"
	fi
	sleep 2s
}
...
update(){
	...
	echo -e "${Info} 开始更新 Snell v5..."
	check_status
	[[ "$status" == "running" ]] && stop
	
	# 备份配置
	if [[ -e ${SNELL_CONF} ]]; then
		cp -f ${SNELL_CONF} "/tmp/snell5_config_backup.conf"
	fi
	
	download
	
	# 恢复配置
	if [[ -e "/tmp/snell5_config_backup.conf" ]]; then
		cp -f "/tmp/snell5_config_backup.conf" ${SNELL_CONF}
		rm -f "/tmp/snell5_config_backup.conf"
	fi
	
	start
	echo -e "${Info} Snell v5 更新完成！"
}

uninstall(){
	check_installed_status
	echo -e "确定要卸载 Snell v5？[y/N]"
	read -e -p "(默认：n)：" unyn
	[[ -z ${unyn} ]] && unyn="n"
	if [[ ${unyn} == [Yy] ]]; then
		check_status
		[[ "$status" == "running" ]] && stop
		systemctl disable snell5
		rm -rf "${SNELL_DIR}"
		rm -f "${SNELL_FILE}"
		rm -f "${SERVICE_FILE}"
		systemctl daemon-reload
		echo -e "${Info} Snell v5 卸载完成！"
	else
		echo -e "${Info} 卸载已取消..."
	fi
}
...
view_log(){
	check_installed_status
	echo -e "${Info} 显示 Snell v5 最近 50 行日志："
	journalctl -u snell5 -n 50 --no-pager
	echo && echo -n " 按回车键返回主菜单..." && read
	start_menu
}
...
start_menu(){
	...
	echo -e "
========================================
   Snell v5 管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
========================================
  
==================状态==================
 Snell v5 Server  : [${status_show}]
========================================
==================菜单==================
 ${Green_font_prefix}1.${Font_color_suffix}  安装 Snell v5
 ${Green_font_prefix}2.${Font_color_suffix}  更新 Snell v5
 ${Green_font_prefix}3.${Font_color_suffix}  卸载 Snell v5
————————————————————————————————————————
 ${Green_font_prefix}4.${Font_color_suffix}  启动 Snell v5
 ${Green_font_prefix}5.${Font_color_suffix}  停止 Snell v5
 ${Green_font_prefix}6.${Font_color_suffix}  重启 Snell v5
————————————————————————————————————————
 ${Green_font_prefix}7.${Font_color_suffix}  修改 Snell v5 配置
 ${Green_font_prefix}8.${Font_color_suffix}  查看 Snell v5 配置
 ${Green_font_prefix}9.${Font_color_suffix}  查看 Snell v5 状态
 ${Green_font_prefix}10.${Font_color_suffix} 查看 Snell v5 日志
————————————————————————————————————————
